...
(function($) {
    doc.ready(function() {
        ...
        new Booklet({width: 900, height: 647});
        ...
        new YMotion([
            ...
            [
                {el: '.el2_2020', set: {opacity: 0, y: 50}, to: {opacity: 1, y: 0}, d: 0.4},
                {el: '.el2_2019', set: {opacity: 0, y: 50}, to: {opacity: 1, y: 0}, d: 0.4},
                {el: '.el2_2018', set: {opacity: 0, y: 50}, to: {opacity: 1, y: 0}, d: 0.4},
                {el: '.el2_2017', set: {opacity: 0, y: 50}, to: {opacity: 1, y: 0}, d: 0.4},
                {el: '.el2_2016', set: {opacity: 0, y: 50}, to: {opacity: 1, y: 0}, d: 0.4},
            ],
        ]).activate();
        var recruitListLoader = function(){
            var matchWrapSelector = '.layer4';
            var waitResponse = false;
            var initURL = '/board/index.php?language=&board=recruit_ad_01&is_ajax=true';
            this.init = function(){
                this.loadURL(initURL);
                this.addEvent();
            };
            this.addEvent = function(){
                $('.paging a', matchWrapSelector).on('click', function(e){
                    if(waitResponse===false)this.loadURL($(this).attr('href'));
                    e.preventDefault();
                });
            };
            this.loadURL = function(url){
                ...
                $.ajax({
                    ...
                    success : function(response){
                        waitResponse = false;
                        $(matchWrapSelector).html($(matchWrapSelector, response));
                    }
                });
            };
            this.init();
